...
def _settings(**overrides: object) -> SimpleNamespace:
    values = {
        ...
        'media_group_max_size': 40,
    }
    values.update(overrides)
    return SimpleNamespace(**values)


def _derived_variant_source_max_duration(settings: SimpleNamespace) -> timedelta:
    return timedelta(
        seconds=math.ceil(
...
async def test_send_variant_audio_single_variant_uses_send_audio() -> None:
    ...
    await track_retrieve_module._send_variant_audio(
        ...
        chat_id=9,
        variants=variants,
        media_group_max_size_mib=40,
    )

    bot.send_audio.assert_awaited_once()
    assert bot.send_audio.await_args.kwargs['audio'].filename == '--'
    assert bot.send_audio.await_args.kwargs['audio'].data == b'only'
    assert bot.send_audio.await_args.kwargs['chat_id'] == 9
...
async def test_send_variant_audio_small_multi_variant_uses_media_group() -> None:
    ...
    assert bot.send_media_group.await_args.kwargs['chat_id'] == 9
    media = bot.send_media_group.await_args.kwargs['media']
    assert [item.media.filename for item in media] == ['++', '--']
    assert [item.media.data for item in media] == [b'first', b'second']


@pytest.mark.asyncio
async def test_send_variant_audio_large_multi_variant_skips_media_group_and_sends_sequential(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    bot = AsyncMock()
    variants = [
    ...
    await track_retrieve_module._send_variant_audio(
        ...
        chat_id=9,
        variants=variants,
        media_group_max_size_mib=1,
    )

    assert bot.send_audio.await_count == 2
    assert [call.kwargs['audio'].filename for call in bot.send_audio.await_args_list] == ['++', '--']
    assert [call.kwargs['audio'].data for call in bot.send_audio.await_args_list] == [b'a' * 700_000, b'b' * 700_000]
    assert all(call.kwargs['chat_id'] == 9 for call in bot.send_audio.await_args_list)
    bot.send_media_group.assert_not_awaited()
...
async def test_send_variant_audio_falls_back_when_media_group_too_large(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    log_warning = Mock()
    monkeypatch.setattr(track_retrieve_module.logger, 'warning', log_warning)
    ...
    assert bot.send_audio.await_count == 2
    assert [call.kwargs['audio'].filename for call in bot.send_audio.await_args_list] == ['++', '--']
    assert [call.kwargs['audio'].data for call in bot.send_audio.await_args_list] == [b'first', b'second']
    assert all(call.kwargs['chat_id'] == 9 for call in bot.send_audio.await_args_list)
...
@pytest.mark.asyncio
async def test_audio_only_batch_dispatches_to_track_menu() -> None:
    scheduler = _FakeScheduler()
    buffer = ChatMessageBuffer()
    services = _services(clip_store=SimpleNamespace(), scheduler=scheduler, buffer=buffer)
    first_message = _fake_message(chat_id=42, message_id=1, audio=_fake_audio(file_id='audio-1', file_name='one.mp3'))
...
async def test_text_and_audio_batch_dispatches_to_track_menu() -> None:
    ...
    services = _services(clip_store=SimpleNamespace(), scheduler=scheduler, buffer=buffer)
    settings = _settings()
    first_message = _fake_message(chat_id=42, message_id=1, text='Artist\nTitle')
    message = _fake_message(chat_id=42, message_id=2, audio=_fake_audio(file_id='audio-1', file_name='one.mp3'))
...
@pytest.mark.asyncio
async def test_supported_youtube_link_only_batch_dispatches_to_track_menu() -> None:
    scheduler = _FakeScheduler()
    buffer = ChatMessageBuffer()
    services = _services(clip_store=SimpleNamespace(), scheduler=scheduler, buffer=buffer)
    settings = _settings()
...
@pytest.mark.asyncio
async def test_unsupported_youtube_like_link_only_batch_shows_fallback_menu() -> None:
    scheduler = _FakeScheduler()
    buffer = ChatMessageBuffer()
    services = _services(clip_store=SimpleNamespace(), scheduler=scheduler, buffer=buffer)
    settings = _settings()
    message = _fake_message(
        chat_id=42,
        message_id=2,
        text='https://www.youtube.com/watch?list=abc\nArtist\nTitle',
    )
...
@pytest.mark.asyncio
async def test_track_store_audio_only_reuses_selected_album_cover(monkeypatch: pytest.MonkeyPatch) -> None:
    settings = _settings()
    selected_album_id = '018f05c1f1a37b348d291f53a1c9d901'
    sibling_album_id = '018f05c1f1a37b348d291f53a1c9d902'
    track_store = SimpleNamespace(
        list_tracks=AsyncMock(
            return_value={
                track_store_module.SubSeason.A: [
                ...
                track_store_module.SubSeason.B: [
                    track_store_module.TrackInfo(
                        id='018f05c1f1a37b348d291f53a1c9d103',
                        album_id='018f05c1f1a37b348d291f53a1c9d903',
                        artists=('artist 3',),
    ...
    buffer = ChatMessageBuffer()
    buffer.append(_fake_message(chat_id=42, message_id=1, text='Artist 1\nTitle 1'), chat_id=42)
    buffer.append(
        _fake_message(chat_id=42, message_id=2, audio=_fake_audio(file_id='audio-1', file_name='one.mp3')),
        chat_id=42,
    )
    services = _services(clip_store=SimpleNamespace(), track_store=track_store, buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=52)
    state = _FakeState()
    bot = SimpleNamespace(
        get_file=AsyncMock(return_value=SimpleNamespace(file_path='audio-path-1')),
        download_file=AsyncMock(return_value=BytesIO(b'audio-1')),
    ...
    assert _keyboard_rows(album_menu) == [['Album B'], ['Album A'], ['Back']]
    assert track_store.store.await_count == 0

    await on_track_store_menu(
        _fake_callback(menu_message),
        TrackStoreCallbackData(action=TrackStoreAction.SELECT, step=TrackStoreStep.ALBUM, value=selected_album_id),
        state,
        services,
        settings,
        bot,
    ...
    assert call_args.args == (expected_group, track_store_module.SubSeason.A)
    stored_track = call_args.kwargs['track']
    assert stored_track.artists == ('Artist 1',)
    assert stored_track.title == 'Title 1'
    assert stored_track.cover is None
    assert stored_track.album_id == selected_album_id
    assert stored_track.audio == FileBytes(data=b'opus-1', extension=Extension.OPUS)
    menu_message.answer.assert_awaited_once_with(text='Done')
    assert services.chat_message_buffer.peek_raw(42) == []


@pytest.mark.asyncio
async def test_track_store_audio_caption_fallback_reuses_selected_album_cover(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    settings = _settings()
    selected_album_id = '018f05c1f1a37b348d291f53a1c9d904'
    track_store = SimpleNamespace(
        list_tracks=AsyncMock(
            return_value={
                track_store_module.SubSeason.A: [
                    track_store_module.TrackInfo(
                        id='018f05c1f1a37b348d291f53a1c9d104',
                        album_id=selected_album_id,
                        artists=('artist',),
                        title='Album A',
                        has_instrumental=False,
    ...
    services = _services(clip_store=SimpleNamespace(), track_store=track_store, buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=520)
    state = _FakeState()
    bot = SimpleNamespace(
        get_file=AsyncMock(return_value=SimpleNamespace(file_path='audio-path-1')),
        download_file=AsyncMock(return_value=BytesIO(b'audio-1')),
    ...
    assert stored_track.artists == ('Artist Caption',)
    assert stored_track.title == 'Title Caption'
    assert stored_track.cover is None
    assert stored_track.album_id == selected_album_id
    assert stored_track.audio == FileBytes(data=b'opus-1', extension=Extension.OPUS)
...
@pytest.mark.asyncio
async def test_track_store_audio_only_dedupes_album_options_in_first_occurrence_order() -> None:
    settings = _settings()
    shared_album_id = '018f05c1f1a37b348d291f53a1c9d911'
    track_store = SimpleNamespace(
        list_tracks=AsyncMock(
            return_value={
                track_store_module.SubSeason.A: [
                    track_store_module.TrackInfo(
                        id='018f05c1f1a37b348d291f53a1c9d201',
                    ...
                    track_store_module.TrackInfo(
                        ...
                        album_id='018f05c1f1a37b348d291f53a1c9d912',
                        artists=('artist',),
                        title='Unique Album',
                        has_instrumental=False,
                    ),
    ...
    services = _services(clip_store=SimpleNamespace(), track_store=track_store, buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=53)
    state = _FakeState()
    ...
    assert state.current_state == TrackStoreFlow.album.state
    album_menu = menu_message.edit_text.await_args.kwargs['reply_markup']
    assert _keyboard_rows(album_menu) == [['Unique Album'], ['First Shared'], ['Back']]


@pytest.mark.asyncio
async def test_track_store_audio_only_uses_only_selected_sub_season_albums() -> None:
    settings = _settings()
    track_store = SimpleNamespace(
        list_tracks=AsyncMock(
            return_value={
                track_store_module.SubSeason.A: [
                    track_store_module.TrackInfo(
                        id='018f05c1f1a37b348d291f53a1c9d301',
                        album_id='018f05c1f1a37b348d291f53a1c9d921',
                        artists=('artist',),
                ...
                track_store_module.SubSeason.B: [
                    track_store_module.TrackInfo(
                        id='018f05c1f1a37b348d291f53a1c9d302',
                        album_id='018f05c1f1a37b348d291f53a1c9d922',
                        artists=('artist',),
    ...
    services = _services(clip_store=SimpleNamespace(), track_store=track_store, buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=54)
    state = _FakeState()
    bot = SimpleNamespace(get_file=AsyncMock(), download_file=AsyncMock())
    ...
    assert state.current_state == TrackStoreFlow.album.state
    album_menu = menu_message.edit_text.await_args.kwargs['reply_markup']
    assert _keyboard_rows(album_menu) == [['Sub-season A Album'], ['Back']]


@pytest.mark.asyncio
async def test_track_store_audio_only_with_albums_opens_album_menu_directly() -> None:
    settings = _settings()
    album_id = '018f05c1f1a37b348d291f53a1c9da01'
    track_store = SimpleNamespace(
        list_tracks=AsyncMock(
            return_value={
                track_store_module.SubSeason.A: [
                    track_store_module.TrackInfo(
                        id='018f05c1f1a37b348d291f53a1c9da02',
                        album_id=album_id,
                        artists=('artist',),
                        title='Album A',
                        has_instrumental=False,
    ...
    services = _services(clip_store=SimpleNamespace(), track_store=track_store, buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=541)
    state = _FakeState()
    bot = SimpleNamespace(get_file=AsyncMock(), download_file=AsyncMock())
    year = date.today().year - 1
    ...
    assert state.current_state == TrackStoreFlow.album.state
    album_menu = menu_message.edit_text.await_args.kwargs['reply_markup']
    assert _keyboard_rows(album_menu) == [['Album A'], ['Back']]


@pytest.mark.asyncio
async def test_track_store_audio_only_cover_source_callback_is_stale_unreachable() -> None:
    settings = _settings()
    album_id = '018f05c1f1a37b348d291f53a1c9db01'
    track_store = SimpleNamespace(
        list_tracks=AsyncMock(
            return_value={
                track_store_module.SubSeason.A: [
                    track_store_module.TrackInfo(
                        id='018f05c1f1a37b348d291f53a1c9db02',
                        album_id=album_id,
                        artists=('artist',),
                        title='Album A',
                        has_instrumental=False,
    ...
    services = _services(clip_store=SimpleNamespace(), track_store=track_store, buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=542)
    state = _FakeState()
    bot = SimpleNamespace(get_file=AsyncMock(), download_file=AsyncMock())
    year = date.today().year - 1
...
@pytest.mark.asyncio
async def test_track_store_audio_only_album_menu_back_returns_to_sub_season_menu() -> None:
    settings = _settings()
    album_id = '018f05c1f1a37b348d291f53a1c9d923'
    track_store = SimpleNamespace(
        list_tracks=AsyncMock(
            return_value={
                track_store_module.SubSeason.A: [
                    track_store_module.TrackInfo(
                        id='018f05c1f1a37b348d291f53a1c9d303',
                        album_id=album_id,
                        artists=('artist',),
                        title='Album A',
                        has_instrumental=False,
    ...
    await on_track_store_menu(
        _fake_callback(menu_message),
        TrackStoreCallbackData(action=TrackStoreAction.BACK, step=TrackStoreStep.ALBUM, value='back'),
        state,
        services,
        settings,
        bot,
    ...
    sub_season_menu = menu_message.edit_text.await_args.kwargs['reply_markup']
    assert _keyboard_rows(sub_season_menu) == [['C', 'None'], ['D', 'B', 'A'], ['Back']]
    _assert_format_kwargs(
        menu_message.edit_text.await_args.kwargs,
        _selected_kwargs(
            'Store', 'West', str(year), '1', prompt='Select sub-season:', message_width=settings.message_width
        ),
    )
    assert state.current_state == TrackStoreFlow.sub_season.state
    assert [buffered_message.message_id for buffered_message in services.chat_message_buffer.peek_raw(42)] == [1, 2]
    track_store.store.assert_not_awaited()
...
@pytest.mark.asyncio
async def test_track_store_audio_only_no_albums_in_selected_sub_season_invalidates() -> None:
    settings = _settings()
    track_store = SimpleNamespace(
        list_tracks=AsyncMock(
            return_value={
                track_store_module.SubSeason.B: [
                    track_store_module.TrackInfo(
                        id='018f05c1f1a37b348d291f53a1c9d401',
                        album_id='018f05c1f1a37b348d291f53a1c9d931',
                        artists=('artist',),
                        title='Other Sub-season',
                        has_instrumental=False,
                    )
    ...
    services = _services(clip_store=SimpleNamespace(), track_store=track_store, buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=66)
    state = _FakeState()
    bot = SimpleNamespace(get_file=AsyncMock(), download_file=AsyncMock())
    year = date.today().year - 1
...
@pytest.mark.asyncio
async def test_track_store_audio_only_invalid_text_metadata_invalidates_before_album_menu() -> None:
    settings = _settings()
    track_store = SimpleNamespace(list_tracks=AsyncMock(), store=AsyncMock())
    buffer = ChatMessageBuffer()
    buffer.append(_fake_message(chat_id=42, message_id=1, text='Title only'), chat_id=42)
    buffer.append(
        _fake_message(chat_id=42, message_id=2, audio=_fake_audio(file_id='audio-1', file_name='one.mp3')),
        chat_id=42,
    )
    services = _services(clip_store=SimpleNamespace(), track_store=track_store, buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=67)
    state = _FakeState()
...
@pytest.mark.asyncio
async def test_track_store_single_audio_invalidates_immediately_without_entering_fsm() -> None:
    settings = _settings()
    track_store = SimpleNamespace(list_tracks=AsyncMock(), store=AsyncMock())
    buffer = ChatMessageBuffer()
    buffer.append(
        _fake_message(chat_id=42, message_id=1, audio=_fake_audio(file_id='audio-1', file_name='one.mp3')), chat_id=42
    )
    services = _services(clip_store=SimpleNamespace(), track_store=track_store, buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=671)
    state = _FakeState()
...
@pytest.mark.asyncio
async def test_track_store_audio_caption_fallback_enters_store_fsm() -> None:
    settings = _settings()
    buffer = ChatMessageBuffer()
    buffer.append(
        _fake_message(
    ...
    services = _services(clip_store=SimpleNamespace(), buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=674)
    state = _FakeState()
...
@pytest.mark.asyncio
async def test_track_store_audio_caption_and_text_is_ambiguous_and_invalid() -> None:
    settings = _settings()
    track_store = SimpleNamespace(list_tracks=AsyncMock(), store=AsyncMock())
    buffer = ChatMessageBuffer()
    buffer.append(_fake_message(chat_id=42, message_id=1, text='Artist\nTitle'), chat_id=42)
    ...
    services = _services(clip_store=SimpleNamespace(), track_store=track_store, buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=675)
    state = _FakeState()
...
@pytest.mark.asyncio
async def test_track_store_audio_caption_and_photo_is_invalid() -> None:
    settings = _settings()
    track_store = SimpleNamespace(list_tracks=AsyncMock(), store=AsyncMock())
    buffer = ChatMessageBuffer()
    buffer.append(
    ...
    services = _services(clip_store=SimpleNamespace(), track_store=track_store, buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=676)
    state = _FakeState()
...
@pytest.mark.asyncio
async def test_track_store_valid_text_audio_enters_store_fsm() -> None:
    settings = _settings()
    buffer = ChatMessageBuffer()
    buffer.append(_fake_message(chat_id=42, message_id=1, text='Artist\nTitle'), chat_id=42)
    buffer.append(
    ...
    services = _services(clip_store=SimpleNamespace(), buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=672)
    state = _FakeState()
...
@pytest.mark.asyncio
async def test_track_store_valid_link_only_enters_store_fsm() -> None:
    settings = _settings()
    buffer = ChatMessageBuffer()
    buffer.append(
        _fake_message(
    ...
    services = _services(clip_store=SimpleNamespace(), buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=980)
    state = _FakeState()
...
def test_parse_link_only_store_input_url_only_requires_metadata() -> None:
    parsed = track_store_execution_module.parse_link_only_store_input('https://www.youtube.com/watch?v=abc123')
    assert parsed.url == 'https://www.youtube.com/watch?v=abc123'
    assert parsed.artists is None
    assert parsed.title is None
    assert parsed.requires_metadata is True


def test_parse_link_only_store_input_explicit_metadata() -> None:
    parsed = track_store_execution_module.parse_link_only_store_input(
        'https://www.youtube.com/watch?v=abc123\nArtist 1\nArtist 2\nTitle'
    )
    assert parsed.url == 'https://www.youtube.com/watch?v=abc123'
    assert parsed.artists == ('Artist 1', 'Artist 2')
    assert parsed.title == 'Title'
    assert parsed.requires_metadata is False


def test_parse_link_only_store_input_two_lines_is_invalid() -> None:
    with pytest.raises(track_store_execution_module.TrackInputError, match='Invalid input'):
        track_store_execution_module.parse_link_only_store_input('https://www.youtube.com/watch?v=abc123\nOnly one')


def test_parse_link_only_store_input_accepts_music_youtube_watch_url() -> None:
    parsed = track_store_execution_module.parse_link_only_store_input('https://music.youtube.com/watch?v=abc123')
    assert parsed.url == 'https://music.youtube.com/watch?v=abc123'
    assert parsed.artists is None
    assert parsed.title is None
    assert parsed.requires_metadata is True


def test_parse_link_only_store_input_accepts_youtu_be_short_url() -> None:
    parsed = track_store_execution_module.parse_link_only_store_input('https://youtu.be/s3_I8CoXVLk')
    assert parsed.url == 'https://youtu.be/s3_I8CoXVLk'
    assert parsed.artists is None
    assert parsed.title is None
    assert parsed.requires_metadata is True


def test_parse_link_only_store_input_accepts_youtu_be_short_url_with_si() -> None:
    parsed = track_store_execution_module.parse_link_only_store_input(
        'https://youtu.be/s3_I8CoXVLk?si=VBd84pDWxFi1SK9R'
    )
    assert parsed.url == 'https://youtu.be/s3_I8CoXVLk?si=VBd84pDWxFi1SK9R'
    assert parsed.artists is None
    assert parsed.title is None
    assert parsed.requires_metadata is True
...
def test_parse_link_only_store_input_rejects_invalid_youtu_be_urls(text: str) -> None:
    with pytest.raises(track_store_execution_module.TrackInputError, match='Invalid input'):
        track_store_execution_module.parse_link_only_store_input(text)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        'https://www.youtube.com/watch?list=abc123\nArtist\nTitle',
        'https://www.youtube.com/watch?v=abc123\nOnly one extra line',
    ],
)
async def test_track_store_invalid_link_only_input_invalidates(text: str) -> None:
    settings = _settings()
    track_store = SimpleNamespace(list_tracks=AsyncMock(), store=AsyncMock())
    buffer = ChatMessageBuffer()
    buffer.append(_fake_message(chat_id=42, message_id=1, text=text), chat_id=42)
    services = _services(clip_store=SimpleNamespace(), track_store=track_store, buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=981)
    state = _FakeState()
...
@pytest.mark.asyncio
async def test_track_store_link_with_extra_message_invalidates() -> None:
    settings = _settings()
    track_store = SimpleNamespace(list_tracks=AsyncMock(), store=AsyncMock())
    buffer = ChatMessageBuffer()
    buffer.append(
    ...
    buffer.append(_fake_message(chat_id=42, message_id=2, text='extra'), chat_id=42)
    services = _services(clip_store=SimpleNamespace(), track_store=track_store, buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=982)
    state = _FakeState()
...
@pytest.mark.asyncio
async def test_track_store_link_plus_audio_invalidates() -> None:
    settings = _settings()
    track_store = SimpleNamespace(list_tracks=AsyncMock(), store=AsyncMock())
    buffer = ChatMessageBuffer()
    buffer.append(
    ...
    services = _services(clip_store=SimpleNamespace(), track_store=track_store, buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=983)
    state = _FakeState()
...
async def test_track_store_photo_and_separate_text_link_invalidates() -> None:
    ...
    services = _services(clip_store=SimpleNamespace(), track_store=track_store, buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=984)
    state = _FakeState()
...
@pytest.mark.asyncio
async def test_track_store_valid_photo_audio_enters_store_fsm() -> None:
    settings = _settings()
    buffer = ChatMessageBuffer()
    buffer.append(
        _fake_message(chat_id=42, message_id=1, photo=_fake_photo(file_id='photo-1'), caption='Artist\nTitle'),
    ...
    services = _services(clip_store=SimpleNamespace(), buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=673)
    state = _FakeState()
...
@pytest.mark.asyncio
async def test_track_store_audio_only_album_selection_becomes_stale_when_buffer_changes() -> None:
    settings = _settings()
    album_id = '018f05c1f1a37b348d291f53a1c9d941'
    track_store = SimpleNamespace(
        list_tracks=AsyncMock(
            return_value={
                track_store_module.SubSeason.A: [
                    track_store_module.TrackInfo(
                        id='018f05c1f1a37b348d291f53a1c9d501',
                        album_id=album_id,
                        artists=('artist',),
                        title='Album A',
                        has_instrumental=False,
    ...
    services = _services(clip_store=SimpleNamespace(), track_store=track_store, buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=68)
    state = _FakeState()
    bot = SimpleNamespace(get_file=AsyncMock(), download_file=AsyncMock())
    year = date.today().year - 1
...
@pytest.mark.asyncio
async def test_track_store_audio_only_missing_album_error_maps_to_invalid_input(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    settings = _settings()
    album_id = '018f05c1f1a37b348d291f53a1c9d951'
    year = date.today().year - 1
    group = track_store_module.TrackGroup(
        universe=track_store_module.TrackUniverse.WEST,
        year=year,
    ...
    track_store = SimpleNamespace(
        list_tracks=AsyncMock(
            return_value={
                track_store_module.SubSeason.A: [
                    track_store_module.TrackInfo(
                        id='018f05c1f1a37b348d291f53a1c9d601',
                        album_id=album_id,
                        artists=('artist',),
                        title='Album A',
                        has_instrumental=False,
        ...
        store=AsyncMock(
            side_effect=ValueError(
                f'Album id {album_id} does not exist in group {group.universe.value}-{group.year}-{int(group.season)}'
            )
        ),
    )
    buffer = ChatMessageBuffer()
    ...
    services = _services(clip_store=SimpleNamespace(), track_store=track_store, buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=69)
    state = _FakeState()
    bot = SimpleNamespace(
        get_file=AsyncMock(return_value=SimpleNamespace(file_path='audio-path-1')),
        download_file=AsyncMock(return_value=BytesIO(b'audio-1')),
...
@pytest.mark.asyncio
async def test_track_store_link_only_reuses_selected_album_cover(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    settings = _settings()
    album_id = '018f05c1f1a37b348d291f53a1ca111'
    year = date.today().year - 1
    track_store = SimpleNamespace(
        list_tracks=AsyncMock(
            return_value={
                track_store_module.SubSeason.A: [
                    track_store_module.TrackInfo(
                        id='018f05c1f1a37b348d291f53a1ca112',
                        album_id=album_id,
                        artists=('artist',),
                        title='Album A',
                        has_instrumental=False,
    ...
    assert download_audio_as_opus.await_count == 0
    assert track_store.store.await_count == 0
    assert state.current_state == TrackStoreFlow.cover_source.state

    _assert_format_kwargs(
        menu_message.edit_text.await_args.kwargs,
        _selected_kwargs(
            'Store', 'West', str(year), '1', 'A', prompt='Select cover source:', message_width=settings.message_width
        ),
    )
    cover_source_menu = menu_message.edit_text.await_args.kwargs['reply_markup']
    assert _keyboard_rows(cover_source_menu) == [['Auto'], ['Album'], ['Back']]
    ...
    assert stored_track.artists == ('Artist 1', 'Artist 2')
    assert stored_track.title == 'Title'
    assert stored_track.audio == FileBytes(data=b'opus-bytes', extension=Extension.OPUS)
    assert stored_track.cover is None
    assert stored_track.album_id == album_id
    menu_message.answer.assert_awaited_once_with(text='Done')
    assert services.chat_message_buffer.peek_raw(42) == []
    assert state.current_state is None
...
@pytest.mark.asyncio
async def test_track_store_link_download_failure_invalidates_buffer(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    settings = _settings()
    album_id = '018f05c1f1a37b348d291f53a1ca211'
    year = date.today().year - 1
    track_store = SimpleNamespace(
        list_tracks=AsyncMock(
            return_value={
                track_store_module.SubSeason.A: [
                    track_store_module.TrackInfo(
                        id='018f05c1f1a37b348d291f53a1ca212',
                        album_id=album_id,
                        artists=('artist',),
                        title='Album A',
                        has_instrumental=False,
    ...
    menu_message.edit_text.assert_awaited_with('Download failed', reply_markup=None)
    assert state.current_state is None
    assert services.chat_message_buffer.peek_raw(42) == []
    track_store.store.assert_not_awaited()
...
async def test_track_store_link_download_timeout_logs_exception_and_invalidates_buffer(
    ...
    settings = _settings()
    album_id = '018f05c1f1a37b348d291f53a1ca213'
    year = date.today().year - 1
    track_store = SimpleNamespace(
        list_tracks=AsyncMock(
            return_value={
                track_store_module.SubSeason.A: [
                    track_store_module.TrackInfo(
                        id='018f05c1f1a37b348d291f53a1ca214',
                        album_id=album_id,
                        artists=('artist',),
                        title='Album A',
                        has_instrumental=False,
    ...
    services = _services(clip_store=SimpleNamespace(), track_store=track_store, buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=9861)
    state = _FakeState()
    bot = SimpleNamespace(get_file=AsyncMock(), download_file=AsyncMock())
    monkeypatch.setattr(
        track_store_execution_module, 'download_audio_as_opus', AsyncMock(side_effect=asyncio.TimeoutError())
    )
    log_exception = Mock()
    monkeypatch.setattr(track_ingest_module.logger, 'exception', log_exception)
...
@pytest.mark.asyncio
async def test_track_store_link_only_cover_source_auto_stores_downloaded_cover(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    settings = _settings()
    album_id = '018f05c1f1a37b348d291f53a1ca311'
    year = date.today().year - 1
    track_store = SimpleNamespace(
        list_tracks=AsyncMock(
            return_value={
                track_store_module.SubSeason.A: [
                    track_store_module.TrackInfo(
                        id='018f05c1f1a37b348d291f53a1ca312',
                        album_id=album_id,
                        artists=('artist',),
                        title='Album A',
                        has_instrumental=False,
    ...
    assert stored_track.title == 'Title'
    assert stored_track.audio == FileBytes(data=b'opus-bytes', extension=Extension.OPUS)
    assert stored_track.cover == FileBytes(data=b'jpg-bytes', extension=Extension.JPG)
    assert stored_track.album_id is None
    menu_message.answer.assert_awaited_once_with(text='Done')
    assert services.chat_message_buffer.peek_raw(42) == []
    assert state.current_state is None
...
@pytest.mark.asyncio
async def test_track_store_link_url_only_cover_source_auto_derives_metadata(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    settings = _settings()
    year = date.today().year - 1
...
@pytest.mark.asyncio
async def test_track_store_link_url_only_metadata_error_invalidates_with_specific_message(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    settings = _settings()
    year = date.today().year - 1
    ...
    services = _services(clip_store=SimpleNamespace(), track_store=track_store, buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=992)
    state = _FakeState()
    bot = SimpleNamespace(get_file=AsyncMock(), download_file=AsyncMock())
    monkeypatch.setattr(
        track_store_execution_module,
...
@pytest.mark.asyncio
async def test_track_store_cover_plus_link_download_failure_invalidates_buffer(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    settings = _settings()
    year = date.today().year - 1
    ...
    services = _services(clip_store=SimpleNamespace(), track_store=track_store, buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=9921)
    state = _FakeState()
    bot = SimpleNamespace(
        get_file=AsyncMock(return_value=SimpleNamespace(file_path='cover-path-1')),
        download_file=AsyncMock(return_value=BytesIO(b'raw-cover-1')),
...
async def test_track_store_album_reuse_link_download_failure_invalidates_buffer(
    ...
    services = _services(clip_store=SimpleNamespace(), track_store=track_store, buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=9931)
    state = _FakeState()
    bot = SimpleNamespace(get_file=AsyncMock(), download_file=AsyncMock())
    monkeypatch.setattr(
        track_store_execution_module, 'download_audio_as_opus', AsyncMock(side_effect=RuntimeError('yt failed'))
...
@pytest.mark.asyncio
async def test_track_store_cover_source_back_returns_to_sub_season_menu() -> None:
    settings = _settings()
    album_id = '018f05c1f1a37b348d291f53a1ca411'
    year = date.today().year - 1
    track_store = SimpleNamespace(
        list_tracks=AsyncMock(
            return_value={
                track_store_module.SubSeason.A: [
                    track_store_module.TrackInfo(
                        id='018f05c1f1a37b348d291f53a1ca412',
                        album_id=album_id,
                        artists=('artist',),
                        title='Album A',
                        has_instrumental=False,
    ...
    await on_track_store_menu(
        _fake_callback(menu_message),
        TrackStoreCallbackData(action=TrackStoreAction.BACK, step=TrackStoreStep.COVER_SOURCE, value='back'),
        state,
        services,
        settings,
        bot,
...
@pytest.mark.asyncio
async def test_track_store_link_only_without_albums_executes_auto_immediately(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    settings = _settings()
    year = date.today().year - 1
...
@pytest.mark.asyncio
async def test_track_store_link_only_missing_group_executes_auto_immediately(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    settings = _settings()
    year = date.today().year - 1
    ...
    track_store = SimpleNamespace(
        list_tracks=AsyncMock(
            side_effect=track_store_module.TrackGroupNotFoundError(
                ...
                year=group.year,
                season=group.season,
                sub_season=track_store_module.SubSeason.A,
            )
        ),
        store=AsyncMock(),
    )
    ...
    buffer.append(
        _fake_message(
            chat_id=42,
            message_id=1,
            text='https://www.youtube.com/watch?v=tDoVzwp-f_g&pp=ygUQYnJpbCBOb3cgaXQgZ29lcw%3D%3D\nA\nB',
        ),
        chat_id=42,
    )
    services = _services(clip_store=SimpleNamespace(), track_store=track_store, buffer=buffer)
